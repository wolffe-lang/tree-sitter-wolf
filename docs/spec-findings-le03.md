...
reading are recorded here with the compiler's parser left unread; each
is filed upstream.
...
already E0110, so seven-plus significant digits refuse themselves;
only leading zeros stay undecided.
