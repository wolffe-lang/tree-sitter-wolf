...
measurement**, and §1 gives the measurement rather than the assumption —
le07's precedent. §3 is the part worth keeping: le07's own §2 recorded a
fact about `char` that is not true, and the reason is a path that does
not exist.
...
header and declares none). Not one new type name, not one new keyword,
not one new operator, not one new literal form.

## 1. The measurement, not the prediction

Three independent limbs, all taken rather than assumed:
...
`wolf_lex` and `wolf_parse` in **exactly eight files, all of them
...
What grew is `PRELUDE` — the builtin FUNCTION list — by exactly the five
names above (`net_listen_with`, `net_adopt_listener`, `net_wait`,
`os_cpus`, `os_spawn_with`). That distinction is the whole sprint: this
repo keeps **no builtin-function list**. `queries/highlights.scm` paints
calls structurally —
...
— never by name, so every one of s137's five calls is highlighted
correctly the moment it is written, with no query edit. le07 recorded
this posture for `[os.net.unix]`'s two calls; s137's five arrive through
the same door, and it is now the second release in a row where the OS
tier grew and this repo did nothing. **That is the design working, not
the sprint being skipped.**
...
The cause is in the quoted command: le07 grepped a directory named
`compiler/`, and **wolf-lang has no `compiler/` directory** — not at
trunk, not at `v0.2.4`, not at `v0.2.5`. The crates live at
`crates/` off the repo root. A recursive grep of a nonexistent path
returns nothing, and le07 read that nothing as an absence of the name.
...
- `Prim::Char` **does** exist — `crates/wolf_mem/src/ubcheck.rs:7735`
  at v0.2.5 (`:7375` at v0.2.4), in three match arms.

So `char` passes **both** limbs of le06's conjunction and always did.
le07's *conclusion* — keep `char` in `@type.builtin` — is correct, and
the query file needs no change today. But its recorded *reason* is a
measurement error, and the sentence "the mid-end has not landed `char`"
is exactly the kind of belief a later sprint acts on. It is retired
here.
...
At wolf-lang trunk `6263ffa` (the corpus is **byte-identical** at
`v0.2.5` and at trunk — `git diff v0.2.5..6263ffa -- corpus` is empty):
**507** `.lu` files, **25** parse-tier counter-examples excluded by
directive, **482** gated, **zero** ERROR nodes. The floor ratchets
**478 → 482**.

The four added files are exactly s137's four witnesses; the exclusion
count is unchanged at 25, since none of the four is a parse-tier
...
- le07's §3, unchanged and still live: the **leading-BOM** tolerance is a
  ...
  `tree-sitter test` harness, and the harness actively disagrees with it.
  Do not "fix" `grammar.js` to satisfy a harness failure on a leading
  BOM. le08 added no BOM tests and changed nothing here; the mid-file
  half stays pinned in `test/corpus/items.txt`, the leading half by
  Gate 4 over `corpus/grammar/bom_at_start.lu`.
- Nothing new. s137 asked this repo for nothing and got nothing.
