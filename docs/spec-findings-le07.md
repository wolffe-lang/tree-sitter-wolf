...
was which of those lines add SYNTAX. The answer is measured below: one
line, and even that one is already satisfied. The measurement that
proves it had to be taken through the right door, and the wrong door
gives the opposite answer; §3 has that measurement.
...
`spec/10-types.md` §4b is 70 new lines, and all of them are typing
rules. The clause settles it in its own words:
...
set. It has been there since le02, was re-checked at le06, and is
correct at v0.2.4. No query change.

## 2. The builtin list re-checked against the compiler's closed set

The le06 pass struck `usize`/`isize` on a two-limb test: no `spec/*.md`
names them AND they are not in the compiler's closed builtin set. Both
limbs were re-run at v0.2.4, because a type-tier release is when that
list drifts.
...
- `byte` is now in the compiler's set, with a `Prim::Byte` beside it
  in `wolf_sema/src/types.rs`. The le06 pass kept `byte` on the spec limb
  alone ("it is a builtin at this tag already"); at v0.2.4 both limbs
  carry it. The le06 line in `highlights.scm` that says this list "does
  not wait on" s135 is retired by events.
- **`char` is in NEITHER limb of the compiler.** `grep -rn '"char"'
  ...
  no oversight; it passes the SPEC limb overwhelmingly (`[type.char]`
...
begin `EF BB BF` after a save. The library's answer matches the spec's,
both halves: position 0 is skipped, every other position is a
stray character. `extras` is `[/\s/, …]` and `/\s/` does not match
U+FEFF, so the tolerance is the tree-sitter runtime's own position-0
skip and not a hole in `extras`, which is why the mid-file half stays
strict.

**`grammar.js` needs no change, and must not get one.** The trap is
worth naming because this port walked into it: the `tree-sitter test`
harness does not perform the runtime's position-0 skip (it feeds a test
body straight in), so three corpus tests written to pin D74's
leading-BOM tolerance all failed, and the obvious "fix" is to add U+FEFF
to `extras` or to open `source_file` with an optional BOM token. Either
...
the mid-file BOM trivia too, retiring E0107's half of D74. The
harness disagrees with both the CLI and the library, and the harness is
...
- The leading half by Gate 4: `corpus/grammar/bom_at_start.lu` is
  one of the 478 gated files and parses at zero ERROR nodes.
- The leading half through the library is pinned by nothing
  ...
  failure is written down here. This is the one known gap carried
  forward.
