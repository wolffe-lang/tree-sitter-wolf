...
spec silence is recorded here and the compiler's parser is left unread.
Each entry is drafted as an upstream issue for wolf-lang; filing is the
integrator's act, one `gh issue create` each.
...
`'(' type (',' type)* ','? ')'`, which needs at least one type, so `()`
cannot be derived, and no other alternative produces a unit type. The
corpus and the spec disagree here.
...
(`corpus/grammar/interp_nested.lu`) does not contain the example, so
nothing catches the drift. Either the example needs a path
...
In `struct_lit ::= path '{' …` the head is a bare path, so a generic
